 ...
 * @author Vaud Keith
 */
public class JFSalesByFoodView extends javax.swing.JFrame {

    /**
     * Creates new form JFSalesByFoodView
     */
    String filePath = "";
    File file;
    
    public JFSalesByFoodView() {
        initComponents();
        setTitle("Sales By Food View");
        setSize(900, 600);
    ...
    private boolean export(){ 
            ...
            try {
                ...
                file = new File(DirectoryManager.getExportDirectory()+"\\SBF"+num+".xlsx");
                filePath = file.toPath().toString();
                ExportData.writeToExcel(table,file.toPath());
                succeeded = true;
            } catch (Exception ex) {
               HError.showErrorMessage("SBF001: Error exporting your data.", null, ex.getMessage());
               succeeded = false;
            }
    ...
    private void populateTable(){
        Driver sqliteDriver;
           ...
           try{
               ...
               String sql = "SELECT DateSold,Product, SUM(Quantity) AS Quantity,PPU,SUM(TotalPrice) AS Total"
                       + " FROM sbf GROUP BY  Product,DateSold ORDER BY DateSold DESC";
               ...
               HError.showErrorMessage("SBF002: Database error.Cant fetch data.", this, ex.getMessage());
           }finally{
               if(rs != null){
                   try{
                       rs.close();
     ...
     private void populateTable(String date){
        Driver sqliteDriver;
           ...
           try{
               ...
               conn = HotelDB.getDBConnection();
               String sql = "SELECT DateSold,Product, SUM(Quantity) AS Quantity,PPU,SUM(TotalPrice) AS Total"
                       + " FROM sbf WHERE DateSold = '" + date + "' GROUP BY Product ORDER BY DateSold DESC";
               stmt = conn.createStatement();
               rs = stmt.executeQuery(sql);
               table.setModel(tableModel(rs));
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(JFSalesByFoodView.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new JFSalesByFoodView().setVisible(true);
            }
        });
    }
